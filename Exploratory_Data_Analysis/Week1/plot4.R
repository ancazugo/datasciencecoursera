...
#Divide device in 2 row and 2 columns
par(mfrow = c(2, 2))

#Create line plot  of active power vs time
plot(Global_active_power ~ Date.Time, data = df_2, type ='l', xlab = '', ylab = 'Global Active Power (kilowatts)')

#Create line plot  of voltage vs time
plot(Voltage ~ Date.Time, data = df_2, type ='l', xlab = 'datetime')
...
#Create line plot of reactive power vs time
plot(Global_reactive_power ~ Date.Time, data = df_2, type = 'l', xlab = 'datetime')
